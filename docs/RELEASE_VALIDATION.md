...
**심사 상태 갱신:** 2026-09-26 13:56 Apple 메시지의 사용자 제공 스크린샷에서 Guideline 2.1 추가 정보 요청을 확인했습니다. [대응 자료](APP_REVIEW_FOLLOWUP.md)의 6개 항목 중 실물 기기 영상이 남아 있고 회신·재제출은 미진행입니다. 특정 코드 오류의 재현 사례가 메시지에 없으므로 이번 대응에서 앱 코드를 변경하거나 새 빌드를 만들지 않았습니다. 기존 미완료 기기 검증을 통과 처리하지 않습니다.
...
아래 항목은 아직 완료하지 않았습니다. 자동 테스트의 관련 사례가 통과한 것과 실제 화면·기기에서 확인한 것은 구분합니다.

### 개발자 프로그램 가입 전에도 진행할 화면·호환성 검증

- [ ] iPad 두 창에서 근무 시작·휴식·종료, 동일 기록의 수정·삭제, 상세 화면을 연 상태에서 초기화를 직접 조작하고 중복 세션·오래된 화면 덮어쓰기가 없는지 확인합니다. 관련 로직은 자동 검증했으나 두 창 UI 시험은 남아 있습니다.
- [ ] VoiceOver 탐색·읽기·실행, 최대 접근성 글씨, 가로 화면에서 온보딩·홈·기록 편집·꾸미기·설정의 전체 흐름을 확인합니다. 이전 후보의 일부 큰 글씨 화면 확인만으로 완료 처리하지 않습니다. 시뮬레이터에서 가능한 항목을 먼저 확인하고 실기기에서도 재확인합니다.
- [ ] iOS 17 / watchOS 10에서 설치·실행·저장·재실행·핵심 조작을 확인합니다. 해당 런타임은 현재 설치되어 있지 않으며, 최소 배포 버전 빌드 성공과 실제 실행 검증은 다릅니다.

### 실물 기기가 필요한 검증

- [ ] iPhone↔Watch 왕복 제어, 연결 끊김·재연결, 백그라운드 통신과 장착 이미지 동기화를 확인합니다.
- [ ] 알림 허용·거부·재허용 후 실제 수신과 휴식·종료 후 취소, 위젯의 시스템 갱신을 확인합니다.
- [ ] 실기기의 배터리·발열·메모리와 장시간 근무·백그라운드 복귀를 확인합니다. 개발용 Mac의 자원 측정은 앱의 실기기 성능 시험을 대체하지 않습니다.
- [ ] 사용 중인 기기 데이터를 백업하고 배포 빌드로 업그레이드해 기록·보유/장착·잔액과 재실행 보존을 확인합니다. 합성 초기 스키마와 기존 시뮬레이터 데이터 이전은 이미 통과했습니다.
...
- [ ] Guideline 2.1 추가 정보 요청에 실물 기기 영상과 설명 6개 항목으로 대응합니다. 영문 초안 준비, 영상·전송·재제출은 대기 중입니다.
- [ ] Apple 승인 후 수동으로 공개 출시합니다.
