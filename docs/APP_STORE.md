...
- App Privacy: 개인정보처리방침 URL과 데이터 미수집 응답을 저장했습니다. 게시 확인창은 답변 정확성·심사 지침/관련 법 준수·처리 지침 변경 즉시 갱신에 대한 동의를 포함하므로 사용자 확인 전에는 게시하지 않았습니다.
- ‘심사에 추가’ 사전 검증 결과: 개인정보 처리지침 정보 제공 한 항목만 남았다고 표시했습니다. 이 검증은 심사 승인이나 전체 기기 시험 통과를 뜻하지 않습니다.

심사 항목 추가는 위 검증에서 차단됐으며, 심사 제출 및 공개 출시는 하지 않았습니다.
...
- App Store Connect 앱 생성/연결·무료 가격·대한민국 단독·연령 등급 입력 완료. 개인정보 게시 동의 및 심사 제출은 남음
- 개인정보·지원 문서의 공개 URL 접속 및 지원 이메일 수신 가능 여부
- 정책의 문의 메일 보관 기간(처리 완료 후 1년 이내) 실제 운영 준수
- 실물 iPhone + Watch 왕복 제어/재연결/강제종료 복구/알림 확인
- TestFlight 빌드와 iPhone/iPad/Watch 스크린샷 연결 완료. 최종 심사 제출은 남음
